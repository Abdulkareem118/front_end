...
const Inventory = () => {
  ...
  const fetchInventory = async () => {
    try {
      const res = await axios.get('https://backend-pos-zps4.onrender.com/api/inventory');
  ...
  const handleAddItem = async (values) => {
    try {
      const res = await axios.post('https://backend-pos-zps4.onrender.com/api/inventory', {
        name: values.name,
        stock: values.stock,
      });
  ...
  const handleSellQuantityChange = async (value, record) => {
    try {
      const res = await axios.put(`https://backend-pos-zps4.onrender.com/api/inventory/${record._id}`, {
        sellQuantity: value,
      });
